package com.bubble.bnlp.classify.decisionTree.id3.client;

import com.bubble.bnlp.classify.decisionTree.id3.core.ID3Core;
import com.bubble.bnlp.classify.decisionTree.id3.model.AttributeNode;
import com.bubble.bnlp.classify.decisionTree.id3.utils.DecisionTreeUtils;
...
public class ID3Client {
    ...
    public void createDecisionTree(ID3Core core, List<List<String>> currentData) {
        Map<String, Double> maxIGMap = core.maxInformationGain(currentData);
        maxIGMap.forEach((k, v) -> {
            System.out.println("best feature = " + k);
            System.out.println("max ig value = " + v);
        });
